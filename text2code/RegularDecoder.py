...
class RegularDecoder(nn.Module):
  def __init__(self, vocabs, opt):
    ...
    self.layer1 = nn.Linear(in_features = opt.tgt_word_vec_size,out_features = opt.tgt_word_vec_size)
  ...
  def forward(self, batch, context, context_lengths, decState):
    ...
    tgt_embeddings = self.layer1(tgt_embeddings)
    tgt_embeddings = self.dropout(tgt_embeddings)
    
    rnn_output, prev_hidden = self.decoder_lstm(tgt_embeddings, decState.hidden)
